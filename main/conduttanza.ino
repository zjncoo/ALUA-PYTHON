...
const int SCL_INPUT0 = A0; // SCL persona A
const int SCL_INPUT1 = A1; // SCL persona B
...
const unsigned long SCL_VALID_DURATION =
    EXPERIMENT_DURATION - SCL_START_DELAY; // 45000 - 5000 = 40000 ms
...
void loopConduttanza() {
  ...
  unsigned long elapsed =
      now - experimentStart; // ms trascorsi dall'inizio esperimento
...
void updateSCL(unsigned long elapsed) {
  ...
  // Questo garantisce che l'AUDIO funzioni sempre e sia reattivo per tutti i
  // 60s.
...
void evaluateSCLTrend() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (meanFirst == 0.0 || meanSecond == 0.0) {
      Serial.println("  Arousal SCL: DATI INSUFFICIENTI (non posso confrontare "
                     "le due metà).");
